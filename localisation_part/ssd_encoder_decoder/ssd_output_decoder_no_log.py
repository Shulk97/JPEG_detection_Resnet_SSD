...
def decode_detections(y_pred,
    ...
    if input_coords == 'centroids':
        y_pred_decoded_raw[:,:,[-2,-1]] = y_pred_decoded_raw[:,:,[-2,-1]] * y_pred[:,:,[-2,-1]] # exp(ln(w(pred)/w(anchor)) / w_variance * w_variance) == w(pred) / w(anchor), exp(ln(h(pred)/h(anchor)) / h_variance * h_variance) == h(pred) / h(anchor)
...
def decode_detections_fast(y_pred,
    ...
    if input_coords == 'centroids':
        y_pred_converted[:,:,[4,5]] = y_pred_converted[:,:,[4,5]] * y_pred[:,:,[-2,-1]] # exp(ln(w(pred)/w(anchor)) / w_variance * w_variance) == w(pred) / w(anchor), exp(ln(h(pred)/h(anchor)) / h_variance * h_variance) == h(pred) / h(anchor)
