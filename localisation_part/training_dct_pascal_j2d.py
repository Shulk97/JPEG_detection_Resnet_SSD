...
parser = ArgumentParser(description="Script to train the SSD on the pascal voc dataset.")
...
loading_check.add_argument("--vgg", action="store_true")
...
from models.keras_ssd300_dct_j2d import ssd_300DCT
from models.keras_ssd300_dct_j2d_no_regularizer import ssd_300DCT_no_reg
...
copyfile("training_dct_pascal.py", os.path.join(os.environ["LOCAL_WORK_DIR"], "training.py"))
...
if args.no_reg:
    model = ssd_300DCT_no_reg(**ssd_args)
elif args.reg:
    model = ssd_300DCT(**ssd_args)
else:
    raise Exception("Network selection error.")
...
if args.weights:
    ...
    ignore = ["pool5", "flatten", "dropout_1", "dropout_2", "fc1", "fc2", "predictions"]
    for i in range(len(temp_model.layers)):
        if temp_model.layers[i].name not in model_layers:
            if temp_model.layers[i].name not in ignore:
                raise NameError("The model you are trying to load contains weights that will not be loaded: {}".format(temp_model.layers[i].name))
            else:
                print("Ignoring layer: {}".format(temp_model.layers[i].name))
